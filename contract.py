...
GRACE_SECONDS: dict[ContentType, int] = {
    ...
    # 닫기도 같은 이유로 넓힌다 (v1.82). **늦게 닫는 것이 안 닫는 것보다 낫다** —
    # 못 닫으면 경기 내내 표를 계속 받아 집계가 더 크게 틀어진다(④ 정확).
    # 실측 2026-09-22: 닫기 1건이 8분 유예를 넘겨 폐기 → 그 투표는 열린 채 남았다.
    ContentType.POLL_CLOSE: 90 * 60,         # v1.9: 1800 → 480 (열린 투표를 오래 두면 안 된다)
