...
────────────────────────────────────────────────────────────────────
★★ **이 채널이 지키는 것 넷** (2026-09-22 대표님 확정)

이것이 **모든 판단의 기준**이다. 새 기능을 넣을지, 어느 쪽으로 고칠지
다툴 때 여기로 돌아온다. 넷 다 못 지키면 **④가 이긴다.**

  ① **풍부한 정보력** — 손님이 알고 싶은 것이 다 있다.
     카드가 짧아 못 담으면 **버리지 말고 글로 내린다**(v1.76).

  ② **어디보다 빠른 정보 업데이트** — 남보다 먼저 안다.
     느린 소스를 쓰고 있으면 그건 **고쳐야 할 결함**이지 사정이 아니다.
     ⚠️ 유예를 넓혀 늦게라도 내보내는 것은 **임시 조치다**
     (NPB 9시간 — `GRACE_BY_LEAGUE`). 진짜 수정은 배선을 고치는 것이다.

  ③ **원하는 경기를 쉽게 찾는 접근성** — 손님이 헤매지 않는다.
     경기마다 문패(앵커) 하나, 그 아래 전부 모음. '오늘의 경기'가 색인이다.
     ⚠️ 댓글은 알림이 안 울린다 — 그래서 본채널에서 길을 열어 줘야 한다
     (승부예측 투표 · 경기 직전 한 줄).

  ④ **정확한 정보** — 틀린 정보가 나가면 신뢰를 잃는다. **되돌릴 수 없다.**
     그래서 이 저장소의 오래된 규율이 전부 여기서 나온다:
       · 모르는 것을 지어내지 않는다 (`first_final_at` · `goal_seen_at`)
       · 반쪽 재료로 카드를 만들지 않는다 (한쪽만 있는 명단은 명단이 아니다)
       · 못 맞춘 팀이 있으면 그 리그 순위표를 통째로 안 낸다
     ★ **둘을 맞바꾸지 않는다** (2026-09-22 대표님 지적: *"빠른 오보보다
     늦은 사실? 안된다 빠르고 정확해야한다 방법은 있어"*).
     맞바꿔야 할 것처럼 보이면, 그건 원칙의 문제가 아니라 **모르는 시간이
     길어서**다. 줄이는 방법은 셋이고 전부 정확을 안 깎는다 —
       · **소스를 겹친다** — 먼저 확인해 주는 쪽을 쓴다(둘 다 지켜본다)
       · **교차로 확인한다** — 둘이 맞으면 즉시 낸다. 어긋날 때만 기다린다
       · **묶어서 기다리지 않는다** — 점수는 확인 즉시, 상세는 뒤따라 붙인다
     그래도 못 줄이면 그때만 ④가 이긴다. **지어내지 않는다**는 절대 안 깎는다.

     ⚠️ **실측 2026-09-22 — 여기서 내가 틀렸다.** "NPB는 네이버가 더 빠르니
     그쪽으로 옮기면 된다"고 적었는데, 나란히 재 보니 **반대였다**:
     npb.jp 가 `final` 인 경기를 네이버는 아직 `9회초`로 준다. 소스를 바꾸는
     것이 답이 아니라 **우리 배선이 6시간 늦게 큐에 올리는 것**이 원인이다.
     (추측을 계약에 적으면 그게 다음 사람의 근거가 된다 — 재고 나서 적는다.)
────────────────────────────────────────────────────────────────────
"""
...
class Goal:
    ...
    """
    minute: int                       # 소스가 준 **완료된 분** — 표기는 goal_clock()이 만든다
    side: str                         # "home" | "away" — 득점한 팀
    name: str                         # 선수명. **읽을 수 있는 표기만** (is_readable_ko)
    own_goal: bool = False
    added: int = 0                    # 소스가 준 **완료된 추가 분**. 없으면 0


# ── 축구 골 시각 — 소스의 방언을 공식 표기로 (v1.34, 2026-09-12) ──────
#
# ★★★ 대표님이 채널에서 잡으셨다: *"경기 결과에 나온 시간과, 실제 골이 들어간
# 시간이 다른 것 같아 … 추가6분에 결승골 들어간것 같은데, 첫골도 3분에 들어간거 같고"*
#
# **소스의 `time`은 '완료된 분'이고, 축구 공식 표기는 '진행 중인 분'이다.**
# 2분 55초에 들어간 골은 세상 모든 기록에서 `3'`이다. 우리는 `2분`이라 적었다.
#
# 실측(2026-09-12, K리그1·EPL·라리가 **51골**을 초 단위까지 대조):
#
#   | 골                | 소스        | 실제 경과 | 공식 표기 |
#   |-------------------|-------------|-----------|-----------|
#   | 클리말라 (서울)   | time=2      | 2:55      | **3′**    |
#   | 모따 (전북)       | time=25     | 25:55     | **26′**   |
#   | 클리말라 (서울)   | time=90 +5  | 95:40     | **90+6′** |
#
#   정규시간 골 **불일치 0건** · 추가시간 골 **불일치 0건**.
#
# ⚠️ **이 규칙을 두 곳에 적지 않는다.** 카드와 텍스트가 각자 계산하면 반드시
#    어긋난다(약점 198). 실제로 그렇게 어긋나 있었다 — 카드는 추가시간을 아예
#    안 그려 `90′`라 적고, 텍스트는 `후반 추가시간 4분`이라 말했다. **한 화면에서
#    두 말이 달랐다**(약점 67). 그래서 판정을 여기 하나로 모은다.
GOAL_FIRST_HALF_END = 45          # 이 분까지가 전반 (소스 기준)
GOAL_SECOND_HALF_END = 90         # 이 분까지가 후반 — 넘으면 연장

# ★★★ **리그마다 다르다.** 이걸 놓칠 뻔했다.
#
# 처음엔 "소스는 언제나 완료된 분을 준다"고 결론짓고 전 리그에 1을 더하려 했다.
# 표본이 **K리그1 하나**였는데 네 리그를 쟀다고 적었기 때문이다(약점 202를 내가 범했다).
# 리그별로 **전수 실측**하니 갈렸다 (2026-09-12):
#
#   | 리그       | 소스   | 대조한 경기                   | 소스 → 공식            |
#   |------------|--------|-------------------------------|------------------------|
#   | **K리그1** | **경과 분** | 전북 1-2 서울 외 25골    | 2→3 · 25→26 · 90+5→90+6 |
#   | EPL        | 공식   | 아스널 3-0 코벤트리           | 15·23·49 **그대로**    |
#   | 분데스리가 | 공식   | 바이에른 5-1 슈투트가르트     | 21·52·55·82·90+2 그대로 |
#   | 라리가     | 공식   | 빌바오 1-3 세비야             | 15·50·57·70 그대로     |
#   | 세리에A    | 공식   | 로마 4-0 피오렌티나           | 27·52·60·86 그대로     |
#   | 리그1      | 공식   | 릴 2-2 PSG                    | 21·84·**90+1·90+5** 그대로 |
#   | 챔피언스   | 공식   | 리버풀 2-1 AT 마드리드        | 17·40·50 그대로        |
#   | MLS        | 공식   | 시카고 2-1 샬럿               | 18·20·68 그대로        |
#
# **가르는 선은 상위 카테고리다.** K리그1만 국내(`kfootball`) 공급자라 중계가
# 초까지 주고 '완료된 분'을 준다. 해외(`wfootball`)는 전부 이미 공식 표기다.
# 리그1 표본이 특히 값지다 — **추가시간도 보정하지 않는다**는 것을 못 박는다.
#
# ⏳ **유로파리그만 미확인이다.** 2026-09-24 개막이라 종료 경기가 0건이었다.
#    챔피언스리그와 같은 공급자·같은 카테고리라 지금 설정(보정 없음)이 맞을
#    가능성이 높지만 **그건 추론이다.** 개막 후 첫 경기로 확인한다.
#
# **모르는 리그는 더하지 않는다.** 그것이 지금까지의 동작이고, 확인된 일곱
# 해외 리그가 그쪽이다. 새 리그를 붙이거나 소스를 바꾸면 **그 리그를 직접 재서**
# 여기 넣는다 — 추론으로 넣지 않는다(§7-79: "원리적으로 그렇다"는 근거가 아니다).
GOAL_MINUTE_IS_ELAPSED: frozenset = frozenset({League.KL1})


def goal_needs_bump(league: "League | None") -> bool:
    """이 리그의 소스는 **경과 분**을 주는가 (표기하려면 1을 더해야 하는가)."""
...
def goal_clock(minute: int, added: int = 0,
    ...
    """골 하나의 **공식 표기**. `(반, 숫자)`를 돌려준다.
        ...
        goal_clock(2, 0, League.KL1)   → ("전반", "3")     # 경과 분 → +1
        goal_clock(90, 5, League.KL1)  → ("후반", "90+6")
        goal_clock(15, 0, League.EPL)  → ("전반", "15")    # 이미 공식 표기
        goal_clock(90, 2, League.EPL)  → ("후반", "90+2")

    **반 판정은 소스 원본 분으로 한다.** 더한 뒤에 판정하면 45분 골이 46이 되어
    **후반으로 넘어간다** — 실측에 `time=45 · added=0`인 골(실제 45:37)이 있었다.
    그 골은 '전반 46분'이 맞지 '후반 46분'이 아니다.

    `league`를 안 넘기면 **더하지 않는다** — 모르면 지금 동작을 지킨다.
    """
    m = max(0, int(minute or 0))
    a = max(0, int(added or 0))
    bump = 1 if goal_needs_bump(league) else 0
    if m > GOAL_SECOND_HALF_END:
        return "연장", str(m + bump)
    half = "전반" if m <= GOAL_FIRST_HALF_END else "후반"
    if a:
        base = GOAL_FIRST_HALF_END if m <= GOAL_FIRST_HALF_END else GOAL_SECOND_HALF_END
        return half, f"{base}+{a + bump}"
    return half, str(m + bump)


def goal_sort_key(minute: int, added: int = 0) -> tuple[int, int]:
    """골을 시간순으로 놓는 키. **추가시간을 빼먹으면 90+1과 90+9가 뒤섞인다.**"""
...
class ContentType(str, Enum):
    ...
    ANCHOR = "anchor"
    # 그날 편성을 **종목마다 한 통씩**으로 (v1.92 · 옛날엔 한 통이었다). 자정 직후 올리고 고정한다.
...
GRACE_SECONDS: dict[ContentType, int] = {
    ...
    #
    # 실측 2026-09-24: **363분 늦어 폐기**됐다(MLB). 6시간(360분)을 3분 넘겼다.
    #   `⏰ 지각으로 버림 boxscore MLB … 예약 05:18 · 363분 지남`
    # 기록실은 **늦어도 거짓이 되지 않는다** — 그 경기의 이닝·투수 기록은
    # 시간이 지나도 그대로다. 속보(늦으면 거짓말)와 다르다.
    # 그날 안에는 반드시 나가도록 12시간으로 넓힌다. 날이 바뀌면 그날
    # `결과` 카드가 대신 말하므로 그 뒤로는 안 내보내는 편이 맞다.
    ContentType.BOXSCORE: 12 * 3600,
    ContentType.DAILY_INDEX: 7200,
    ContentType.START_ALERT: 480,        # v1.9: 180 → 480 (감시 임계보다 크게)
    # 닫기도 같은 이유로 넓힌다 (v1.82 → v1.97에서 한 번 더).
    # **늦게 닫는 것이 안 닫는 것보다 낫다** — 못 닫으면 경기 내내 표를 계속
    # 받아 집계가 더 크게 틀어진다(④ 정확).
    #
    # 실측 2026-09-22: 8분 유예를 넘겨 폐기 → 투표가 열린 채 남았다 → 90분으로.
    # 실측 2026-09-24: **93분 늦어 또 폐기**됐다(MLS). 3분 차이다.
    #   `⏰ 지각으로 버림 poll_close MLS … 예약 10:30 · 93분 지남`
    #   유예를 실행 간격보다 좁게 잡으면 **교대 한 번에 그대로 죽는다.**
    #
    ...
    # v1.18: 540 → 1740. 리드(30분)보다 1분 짧게 유지해 **경기 시작 이후 발송을
    # 구조적으로 0**으로 남긴다. 창 29분은 실측 최대 공백 12.2분의 2.4배다.
    ContentType.KICKOFF: 1740,
    ...
    # **v1.18b (2026-09-08) — 3600 → 21600(6시간). 대표님: "한계는 항상 해결 가능해".**
    #
    # 앞서 나는 "깃허브가 4시간 늦으면 어떤 창으로도 못 덮는다"고 적었다. **틀렸다.**
    # 못 덮는 것은 **임박 알림**뿐이다 — "10분 뒤 시작"은 경기가 시작되면 의미가
    # 사라지므로 원리적으로 창을 못 넓힌다. 그러나 **결과 속보는 다르다.**
    #
    # 이 카드의 머리말은 점수와 그 경기의 이야기를 말하지 **"방금 끝났다"고 말하지
    # 않는다**(`for_single_result`). 늦게 나가도 거짓이 되지 않는다 —
    # 라인업 카드에 쓴 것과 같은 논리다.
    #
    # 6시간은 실측 최악 공백 240분의 1.5배다. 이제 **깃허브가 4시간 죽어도
    # 그 경기의 결과는 반드시 나간다.** 임박 알림은 못 살리지만,
    # **그 경기가 아무 데도 안 실리는 일은 없어진다.**
    ContentType.FINAL_FLASH: 21600,
    ...
    # ── 투표는 속보가 아니다 (v1.82에서 바로잡음) ─────────────────
    # v1.78에서 이 값을 **바로 위 득점·구간 속보에서 그대로 베꼈다.** 그런데
    # 둘은 성격이 다르다: 속보는 지나면 뜻을 잃지만, **투표는 안 그렇다.**
    # 킥오프 3시간 전 예약이 한 시간 늦어도 여전히 두 시간 전이다.
    #
    # 실측 2026-09-22 — 걸린 투표 5건이 **전부** 죽었다(장부 원문):
    #     지각 폐기 — 예약보다 34분 / 32분 / 31분 / 30분 / 30분 늦음
    # 30분 유예를 0~4분 넘겨 전멸했다. 시계 로그는 그 이유를 이미 적고
    # 있었다: `[발송 창] poll(40<90분)` — 살아남을 창이 앞창 10분 + 유예
    # 30분 = **40분**인데, 실행 교대 같은 자리에서 시계 간격이 그보다 벌어진다
    # (같은 로그: `시계가 60분마다 도는데 발송 창이 좁습니다`).
    #
    # 90분으로 넓힌다 — 최악 간격을 넘기고, 늦어도 킥오프 90분 전에는 열린다.
    # 더 넓히지 않는 이유: 킥오프에 너무 붙어 열리는 투표는 던질 시간이 없다.
    ContentType.POLL: 90 * 60,
...
# **v1.18 (2026-09-08) — 600 → 1800. 대표님 지시: "누락되는 알림이 절대 발생되면 안되".**
#
# 9분 창은 **구조적으로 뚫린다.** 실측 시계 간격은 중앙 5.4분이지만 최대 12.2분이고,
# 창(9분)보다 공백(12.2분)이 크면 그 사이에 시작하는 경기는 반드시 사라진다.
# 실제로 2026-09-08 KBO 5경기(18:30)의 시작 알림이 한 장도 안 나갔다.
#
# **문구는 거짓이 되지 않는다** — 남은 시간을 **보내는 순간에 다시 계산하기** 때문이다
# (`REJUDGE_AT_SEND`). 30분 전에 나가면 "30분 뒤 시작"이라고 적힌다.
# **경기 시작 이후 발송은 여전히 구조적으로 불가능하다** — 유예가 리드보다 1분 짧고,
# 렌더(`kickoff_card`)가 남은 시간 ≤ 0이면 카드를 만들지 않는다.
#
# ⚠️ **대표님이 처음 정하신 규격은 "경기 시작 5~10분 전"이었다.** 이 값은 그보다
# 이르다. 나중 지시("누락 절대 금지")가 더 강하다고 판단해 바꿨고, 대표님께
# 이 사실을 알렸다. 되돌리려면 이 값과 아래 유예를 함께 600/540으로 되돌린다.
KICKOFF_LEAD_SECONDS = 1800
...
SAFETY_NET_FOR: dict["ContentType", "ContentType"] = {
    # ⚠️ **킥오프는 여기서 뺐다 (v1.18, 2026-09-08).**
    #
    # 여기 `START_ALERT`라고 적혀 있었는데 **그 콘텐츠는 2026-09-07에 껐다.**
    # 즉 계약은 안전망이 있다고 말하는데 실제로는 없었고, 검사는 창 넓이만 보고
    # 통과시켰다. 그래서 2026-09-08 KBO 5경기 시작 알림이 사라졌을 때
    # **아무 경고도 뜨지 않았다.** 거짓 안전망은 없는 안전망보다 나쁘다 —
    # 있다고 믿게 만들기 때문이다.
    #
    # **대신 킥오프는 자기 창을 넓혔다**(9분 → 29분, 실측 최대 공백의 2.4배).
...
# **창을 최악 간격까지는 넓힐 수 없는 콘텐츠** (v1.18에서 안전망과 분리했다).
#
# 전에는 이것을 `SAFETY_NET_FOR`에서 파생했다 — "좁은 것은 곧 안전망이 있는 것"이라는
# 전제였다. 그 전제가 깨졌다: 킥오프는 창을 못 넓히는데 **안전망이 없다**
...
#
# 흔한 공백(실측 최대 12.2분)은 창을 넓혀 덮었다. 그러나 **깃허브 액션이
# 통째로 늦는 최악 240분은 어떤 창으로도 못 덮는다** — 4시간 전에 보내는
# 것은 '곧 시작' 알림이 아니기 때문이다. 시계가 안 도는 동안은 창의 문제가
# 아니라 **시계의 문제**다.
#
# 그래서 "절대 누락 금지"를 이렇게 나눈다:
#   ① 흔한 공백 → 창으로 덮는다 (킥오프 9분 → 29분)
#   ② 드문 대형 공백 → 못 덮는다. 대신 **놓쳤다는 사실을 반드시 알린다**
# **모르는 누락이 가장 나쁘다.** 안 나간 것은 다음에 고칠 수 있지만,
# 안 나간 줄 모르는 것은 고칠 기회 자체가 없다.
MUST_ALERT_ON_MISS: frozenset = frozenset({
...
def game_scope(game) -> str:
    ...
    **두 곳에서 각자 만들면 반드시 어긋난다**(약점 45). 여기 한 곳만 둔다.
    ...
    ★ **소스가 경기 이름을 바꾸는 리그는 신원으로 센다** (v1.77).
    npb.jp 는 박스스코어를 올리며 이름을 바꾼다
    (`20260920-YOG-YAK` → `scores-2026-0920-g-s-23`). 이름이 바뀌면 멱등키가
    바뀌고 **같은 경기가 새 경기처럼** 큐에 올라, 예약 시각은 6시간 전
    그대로라 그 자리에서 버려졌다 — NPB 종료 속보가 나흘간 0건이었던 이유다
    (실측 2026-09-22: 여섯 경기 전부 361~366분 늦음).

    ⚠️ **선 이전 경기는 옛 칸 그대로 둔다.** 칸을 바꾸면 이미 보낸 것이
    안 보낸 것처럼 보여 처음부터 다시 나간다.

    ⚠️ 여기를 넓히면 `legacy_kickoff_scope`·`unqueued_*` 같은 **의무 대조도
    같이 따라온다** — 그래야 큐와 대조가 같은 경기를 같은 경기로 센다.
    그것이 이 함수를 한 곳에 둔 이유다.
    """
    gid = game.game_id
...
# 240분 공백에서 각각이 어떻게 되는지 창으로 계산하면:
#
#   "경기가 있다"   경기 예고(창 6시간) · 분석(9시간)  → **덮는다**
#   "곧 시작한다"   킥오프(29분)                        → 못 덮는다(원리적으로)
#   "끝났다"        종료 속보(6시간) · 정리판(6시간)    → **덮는다** (v1.18b에서 넓힘)
#
# 즉 시계가 4시간 죽어도 **그 경기가 아무 데도 안 실리는 일은 없다.**
# 사라질 수 있는 것은 임박 알림 한 장뿐이고, 그것은 `MUST_ALERT_ON_MISS`가
# 반드시 알린다. **모르는 누락이 0이 되는 것이 이 설계의 목표다.**
INFO_CHAIN_MIN_WINDOW_SECONDS = 240 * 60      # 실측 최악 공백

assert all(
    send_window_seconds(_ct, 3600) >= INFO_CHAIN_MIN_WINDOW_SECONDS
    for _ct in (ContentType.LEAGUE_RESULT, ContentType.FINAL_FLASH)), (
    "경기 결과를 담는 카드 중 최악 공백을 못 덮는 것이 있습니다 — "
    "시계가 멎으면 그 경기가 아무 데도 안 실립니다")

assert NARROW_BY_DESIGN <= (frozenset(SAFETY_NET_FOR) | MUST_ALERT_ON_MISS), (
    "창이 좁은데 안전망도 없고 놓쳤을 때 알리지도 않는 콘텐츠가 있습니다 — "
    "그건 대가가 아니라 그냥 누락입니다")
...
# ★★★ **앵커에는 버튼을 달지 않는다** (v1.56 — 2026-09-18 실채널).
#
# 앵커는 채널에 나가는 그 경기의 문패이고, 손님이 **토론방으로 들어가는
# 유일한 문**이다. 그 문은 텔레그램이 직접 그리는 `댓글 남기기` 바다.
#
# 우리가 인라인 버튼을 달자 **그 바가 사라졌다**(대표님 화면 확인: 버튼 둘만
# 보이고 댓글 바가 없다). 버튼을 두 번 고쳐 봤지만 둘 다 안 열렸다:
#   ① `t.me/c/<내부번호>/…?comment=` — 비공개 꼴이라 텔레그램이 무시
#   ② `t.me/<공개아이디>/…?comment=` — 공개로 바꾼 뒤에도 안 열림
#
# **문을 새로 만들지 말고, 원래 있던 문을 막지 않는다.** 안내 문구는 캡션이
...
# 앵커 캡션에 넣는 안내 한 줄. 버튼 대신 이것이 길을 알려 준다.
ANCHOR_THREAD_HINT = "👇 아래 댓글에서 이 경기의 분석 · 선발 · 득점 · 결과를 봅니다"
...
# ★ **2026-09-22: NPB 예외를 걷어냈다.** 대표님: *"빠른 오보보다 늦은
#   사실? 안된다 빠르고 정확해야한다."* 유예를 9시간으로 넓힌 것은 6시간
#   늦은 '속보'를 내보내는 조치였고, 그건 ②원칙을 어긴다.
#   늦게 내보내는 대신 **늦는 원인을 없앴다** — `stable_game_key` 를 보라.
#   표는 비워 두되 **구조는 남긴다**: 소스가 정말 느린 리그가 나오면
#   그때 한 줄 적으면 되고, 그 판단을 여기 한 곳에서만 하게 된다.
...
#
# ★ **NPB 종료 속보가 나흘간 사라진 진짜 원인.**
#
# npb.jp 는 경기가 끝나고 **박스스코어 페이지를 올릴 때** 그 경기의 이름을
# 바꾼다 — `20260920-YOG-YAK` → `scores-2026-0920-g-s-23`.
# 이름이 바뀌면 우리 멱등키도 바뀌고, **같은 경기가 새 경기처럼 보여**
# 그때 큐에 처음 오른다. 예약 시각은 '종료를 처음 안 시각'(=진짜 종료 시각)
# 그대로라, 큐에 오르는 순간 이미 6시간이 지나 있다.
#
# 실측 2026-09-22 (원장 역산, 9-20 여섯 경기 전부):
#     종료 인지 16:54 → 큐 22:57 (363분)   ·   17:28 → 23:29 (361분)
#     17:34 → 23:35 (361분)   ·   21:10 → 03:16 (366분)
#     21:59 → 04:01 (362분)   ·   01:58 → 08:01 (363분)
# 여섯 건이 전부 361~366분 — 무작위가 아니라 **박스스코어 게시 지연**이다.
#
# → **경기를 이름이 아니라 신원으로 센다.** 날짜 + 양 팀이면 같은 경기다.
#   그러면 소스가 이름을 바꿔도 새 항목이 안 생기고, 종료 속보는 **끝난
#   그 시각에** 나간다. 늦게 내보내는 게 아니라 늦는 원인을 없앤 것이다.
#
...
def stable_game_key(game) -> Optional[str]:
    """소스가 이름을 바꿔도 안 흔들리는 그 경기의 칸. 못 만들면 None.

    `game_identity` 와 **같은 재료**를 쓴다(날짜·홈·원정·더블헤더 차수) —
    둘이 갈라지면 한쪽은 같은 경기라 하고 다른 쪽은 아니라고 한다.
    """
    ident = game_identity(game)
    if ident is None:
        return None
    day, home, away, seq = ident
    return f"g:{day}:{away}:{home}" + (f":{seq}" if seq else "")
...
def team_name(team: "TeamRef") -> str:
    ...
    ★ **교정표를 여기서도 거친다 (v1.74).** 여태 `cards_v5._nm()` 만 거쳐서
    **같은 팀이 화면마다 다른 이름으로 나갔다** — 카드에는 `샌디에이고`,
    본문 글에는 `샌디에고`. 표기를 정하는 곳이 둘이면 반드시 갈린다.
    이제 한 곳이다.

    ⚠️ **소스를 조회할 때는 이 함수를 쓰지 않는다.** 조회는 소스가 부르는
    이름으로 해야 한다(`adapters.naver_game._src_name`). 둘을 섞으면
    `AS로마`로 네이버를 뒤지게 되어 그 경기가 통째로 사라진다.
    """
    return fix_team_name(
        TEAM_NAMES.get(team.league, {}).get(team.team_code, team.team_code))
...
#   ③ 표기가 깨져 있다 (군더더기 공백 등)
#   ④ **소스가 팀을 못 알아보게 줄였다** (v1.74, 2026-09-20 대표님 지적:
#      *"보통 인터밀란 · AS로마라고 부르는데 팀명이 조금 다르게 표기되는
#      것들이 많은 것 같네"*). 짧은 표기 자체는 문제가 아니다 — `맨유`·
#      `토트넘`처럼 한국에서 그렇게 부르면 그게 맞다. ④에 드는 것은
#      **한 낱말이 잘려 정체를 잃었거나, 같은 도시의 두 팀이 구분되지 않는**
#      경우뿐이다. 유럽 96팀을 전수로 훑어 **네 건**만 골랐다.
...
TEAM_NAME_FIX: dict[str, str] = {
    ...
    "보되 / 글림트": "보되/글림트",
    # ④ **세리에A에 밀라노 팀이 둘이다.** 소스는 하나를 `밀란`, 다른 하나를
    #    `인테르`라고 준다 — 한 카드에 나란히 놓이면 어느 쪽이 어느 쪽인지
    #    모른다(밀라노 더비는 실제로 그렇게 나간다). 접두사가 곧 구분이다.
    "밀란": "AC밀란",
    "인테르": "인터밀란",
    # ④ 로마도 같은 이유 + 이탈리아에 `로마`라는 지명이 따로 있다.
    #    **AS다 — AC가 아니다**(AC는 밀란 쪽 접두사다).
    "로마": "AS로마",
    # ④ `크리스털`은 클럽 이름이 아니라 **첫 낱말만 잘린 것**이다.
    #    8자라 카드 폭(TEAM_NAME_MAX_LEN)에 정확히 들어간다.
    "크리스털": "크리스털 팰리스",
}
