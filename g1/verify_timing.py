...
# ⚠️ **이 표본은 예고 리드에 매여 있다.** 리드가 120분일 때는 14:10(뉴욕)
# 경기의 예고가 새벽 01:10에 떨어져 심야 회피가 걸렸다. 리드를 210분으로
# 늘리자(예고가 앵커보다 먼저 나오도록) 같은 경기의 예고가 전날 23:40이 되어
# **회피가 안 걸린다** — 시험이 깨진 것이지 코드가 깨진 게 아니다.
# 리드가 길어질수록 심야에 걸리는 경기가 줄어드는 건 오히려 좋은 일이다.
# 회피 자체를 지키려면 **여전히 새벽에 떨어지는** 경기로 시험해야 한다.
_pv_case("한국시각 새벽 경기",
         [mk(League.MLB, "2026-09-04", 17, 10, tz="America/New_York",
