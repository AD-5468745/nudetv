...
(약점 107 — '옛 e스포츠 리그 공식 결과'라 적었는데 실제로는 팬 위키였다).
...
# 30일씩 두 번이면 끝난다.
#
# ── ★ **5대리그도 통째로 빈다 — "매주 돈다"는 틀린 전제였다** (v2.00) ──
#
# 이 자리에는 원래 *"5대리그는 매주 도니까 넓힐 이유가 없다"* 고 적혀 있었다.
# 실측 2026-09-25: EPL·라리가·세리에A·분데스리가·리그1이 **9-21 이후
# 한 경기도 없고 다섯이 똑같이 10-10 에 재개**한다(A매치 휴식기 18일 공백).
# 겨울 휴식기는 이보다 더 길다(분데스리가는 4주 안팎).
#
# 창이 10일이면 그 18일 동안 **미래 편성이 0건**이 되고, 커버리지 감시의
# v1.23 완화 조건(`ahead >= FUTURE_OK_MIN` = "소스가 살아 있다는 증거")이
# 꺼져서 **주말마다 다섯 리그가 빨간불**을 켠다 — 소스는 멀쩡한데.
# 사람이 사실과 반대로 알게 되는 것이라 심각도가 높다.
#
# 대항전과 같은 처방을 같은 값으로 쓴다. 휴식기 어느 날에 조회해도
# 다음 라운드가 창 안에 들어오므로, 0건이 다시 '진짜 신호'가 된다.
AHEAD_DAYS: dict = dict.fromkeys(
    (League.UCL, League.UEL, League.EPL, League.LALIGA,
     League.SERIEA, League.BUNDESLIGA, League.LIGUE1), 50)
