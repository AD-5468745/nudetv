...
# 조건(`ahead >= FUTURE_OK_MIN` = 3)이 꺼져 **없는 사고로 빨간불**이 켜진다.
# 실측 2026-09-26: K리그1 스냅샷이 09-27 까지뿐이라 `[KL1] 오늘 편성이
# 사라짐` 이 빨간불로 올라오고 틱이 실패로 끝났다 — 10월 편성은 멀쩡했다.
# 유럽에서 같은 병을 v2.00 으로 고쳤다(창 10일 → 50일).
_fm21 = lambda d: T.fetch_months(datetime.fromisoformat(d + "T00:00:00+00:00"))
check("★★ 월 하순에는 **다음 달**도 긁는다 (남은 경기가 셋 아래로 안 떨어진다)",
      "10" in _fm21("2026-09-26")[1] and "10" in _fm21("2026-09-17")[1],
      f"{_fm21('2026-09-26')}")
check("  ↳ (변이) 이틀만 보면 9-26 에 다음 달을 못 본다 — 그게 사고였다",
      (datetime.fromisoformat("2026-09-26T00:00:00+00:00")
       + timedelta(days=2)).month == 9)
check("  ↳ 월 상순에는 안 긁는다 (쓸데없이 늘리지 않는다)",
      "10" not in _fm21("2026-09-10")[1], f"{_fm21('2026-09-10')}")
check("  ↳ 지난달은 그대로 본다 (1일 아침에 어제 결과가 필요하다)",
      "08" in _fm21("2026-09-10")[1])
check("  ↳ 해를 넘는 달은 안 넣는다 (소스가 작년을 긁는다)",
      _fm21("2026-12-20")[1] == ["11", "12"], f"{_fm21('2026-12-20')}")
...
# `channel_ref` 는 멱등이 아니라 한 번 더 해시했고(`ch150aae61cb00` →
# `ch02da299356c0`), 별칭 다섯 줄이 **아무도 조회하지 않는 죽은 주소**로
# 적혔다. 더 나빴던 것은 **확인까지 같은 실수를 해서 통과한 것**이다 —
# 양쪽에 같은 잘못된 전제를 넣고 재면 아무것도 안 잡힌다.
_fp217 = C.channel_ref("-1009999999999")
check("★★★ 이미 지문인 값을 다시 걸면 **막는다**",
      _raises217(_fp217),
      "막지 않으면 죽은 주소가 조용히 만들어진다")
check("  ↳ 정상 값은 그대로 동작한다 (게이트가 일을 막지 않는다)",
      C.channel_ref("-100999").startswith("ch")
      and len(C.channel_ref("-100999")) == 14)
...
# 뒤의 진짜 줄들이 밀려 안 보였다. 개수로 접되 **정보는 안 버린다.**
check("★★ 똑같은 줄은 개수로 접는다 (앞자리를 안 뺏는다)",
      T._fold_lines(["A", "B", "A", "A", "C"]) == ["A (3건)", "B", "C"],
      f"{T._fold_lines(['A', 'B', 'A', 'A', 'C'])}")
check("  ↳ 순서는 처음 나온 차례 그대로다",
      T._fold_lines(["Z", "A", "Z"])[0].startswith("Z"))
check("  ↳ 한 번뿐인 줄에는 개수를 안 붙인다 (없던 말을 만들지 않는다)",
      T._fold_lines(["한 번"]) == ["한 번"])
check("  ↳ 빈 목록·None 도 견딘다", T._fold_lines([]) == []
      and T._fold_lines(None) == [])
