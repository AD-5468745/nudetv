...
# 실제로 옛 e스포츠 리그 수집 실패 하나로 이 상황이 벌어질 뻔했다.
...
check("첫 알림은 나간다", _as.alert("점검", ["옛 e스포츠 리그 실패"]) and _n() == 1)
check("같은 내용은 다시 안 나간다", (not _as.alert("점검", ["옛 e스포츠 리그 실패"])) and _n() == 1)
...
check("새 컨테이너에서도 같은 알림은 막힌다 (기록이 파일에 있다)",
      (not _as2.alert("점검", ["옛 e스포츠 리그 실패"])) and _n() == 2)
check("유예를 0으로 주면 항상 나간다 (긴급용 통로는 남긴다)",
      _as2.alert("점검", ["옛 e스포츠 리그 실패"], repeat_after=0) and _n() == 3)
...
check("기록에 알림 본문이 남지 않는다 (지문과 시각뿐)",
      "옛 e스포츠 리그" not in _txt and "점검" not in _txt, _txt[:80])
...
#   21:29  옛 e스포츠 리그: 캐시로 버팀(묵은 데이터) 2.0시간 전 스냅샷
#   21:40  옛 e스포츠 리그: 캐시로 버팀(묵은 데이터) 2.2시간 전 스냅샷
_ct = Fake()
...
_LEGACY_LEAGUE = "옛 e스포츠 리그: 캐시로 버팀(묵은 데이터) {}시간 전 스냅샷"
check("리밋에 걸린 첫 순간은 알린다",
      _cs.alert("시계 점검 필요", [_LEGACY_LEAGUE.format("2.0")]) and _cn() == 1)
check("경과 시간만 늘어난 같은 상태는 되풀이하지 않는다",
      not any(_cs.alert("시계 점검 필요", [_LEGACY_LEAGUE.format(v)])
              for v in ("2.2", "2.4", "3.0", "9.9")) and _cn() == 1,
      f"{_cn()}통")
check("자릿수가 바뀔 만큼 나빠지면 바로 알린다 (조용해지는 게 아니다)",
      _cs.alert("시계 점검 필요", [_LEGACY_LEAGUE.format("30.1")]) and _cn() == 2)
...
check("지문 정규화가 낱말까지 뭉개지는 않는다",
      _fp_norm("옛 e스포츠 리그 실패 3건") != _fp_norm("NPB 실패 3건"))
...
check("  ↳ 그래도 경과 시간은 여전히 뭉갠다 (도배 방지가 풀리면 안 된다)",
      _fp_norm("[옛 e스포츠 리그] 캐시로 버팀 2.0시간")
      == _fp_norm("[옛 e스포츠 리그] 캐시로 버팀 2.4시간"))
...
# 틱마다 기록 수집(30분)·흐름 보강·옛 e스포츠 리그 백오프(6시간) 주기가 달라 어떤 줄은
...
_REAL_LINES = [
    ["기록 수집 실패 — KBO: GateError 순위 불연속 [1,2,3,4,5,6,7,8,8,10]",
     "시각을 놓쳐 취소 1건 — start_alert 옛 e스포츠 리그:2026-09-06 (예약보다 120분 늦어 취소)",
     "묵은 데이터 — 옛 e스포츠 리그: 스냅샷이 36.4시간 묵었습니다",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.3시간 묵었습니다",
     "묵은 '예정' 1건 예) 옛 e스포츠 리그 2026-09-05",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 20.9시간 전"],
    ["수집 실패 — 옛 e스포츠 리그: 팬 위키 리밋 캐시 36.5시간",
     "일시적 실패(자동 재시도) — MLB: 흐름 최종 점수가 우리와 다름 1건",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.4시간 묵었습니다",
     "묵은 '예정' 1건 예) 옛 e스포츠 리그 2026-09-05",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.0시간 전"],
    ["묵은 데이터 — 옛 국제대회: 스냅샷이 22.5시간 묵었습니다",
     "묵은 '예정' 1건 예) 옛 e스포츠 리그 2026-09-05",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.1시간 전"],
    ["기록 수집 실패 — KBO: GateError 순위 불연속 [1,2,3,4,5,6,7,8,8,10]",
     "일시적 실패(자동 재시도) — MLB: 흐름 최종 점수가 우리와 다름 1건",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.6시간 묵었습니다",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.1시간 전"],
    ["수집 실패 — 옛 e스포츠 리그: 팬 위키 리밋 캐시 36.8시간",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.7시간 묵었습니다",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.3시간 전"],
    ["일시적 실패(자동 재시도) — MLB: 흐름 최종 점수가 우리와 다름 1건",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.8시간 묵었습니다",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.3시간 전"],
    ["기록 수집 실패 — KBO: GateError 순위 불연속 [1,2,3,4,5,6,7,8,8,10]",
     "묵은 데이터 — 옛 국제대회: 스냅샷이 22.9시간 묵었습니다",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.4시간 전"],
    ["일시적 실패(자동 재시도) — MLB: 흐름 최종 점수가 우리와 다름 1건",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.5시간 전"],
    ["묵은 데이터 — 옛 국제대회: 스냅샷이 23.0시간 묵었습니다",
     "[record:KBO] 수집이 멈춤 — 마지막 성공 21.6시간 전"],
    ["기록 수집 실패 — KBO: GateError 순위 불연속 [1,2,3,4,5,6,7,8,8,10]",
     "일시적 실패(자동 재시도) — 옛 e스포츠 리그: 캐시로 버팀 0.2시간",
