...
if _thin:
    ...
    # ★ **자리가 바뀌었을 뿐 빠지지 않았다** (v1.76).
    #   대표님: *"이미지카드로는 간단한 요약만 · 상세내용은 텍스트로."*
    #   최근 폼은 이제 카드가 아니라 **캡션의 펼침 글**에 실린다.
    #   검사를 지우지 않고 **보는 자리만 옮긴다** — 지우면 다음에 이 블록이
    #   통째로 사라져도 아무도 모른다.
    check("  ↳ 최근 폼이 **텍스트로** 실린다 (카드에서 내려온 자리)",
          "최근" in " ".join(_tp) and "<blockquote" in " ".join(_tp),
          " ".join(_tp)[:120])
    check("  ↳ 그리고 카드에는 더 이상 없다 (같은 것을 두 번 말하지 않는다)",
          'class="fm"' not in _th)
...
# ═════════════════════════════════════════════════════════════
print("\n7.6 ★★★ 만들어지는 것과 '나가는 것'은 다르다 — 실제 렌더까지 태운다")
# ═════════════════════════════════════════════════════════════
#
# **이 절이 없어서 대표님이 옛 디자인을 받으셨다 (2026-09-08).**
#
# 위 검사들은 HTML을 만들기만 하고 끝난다. 그런데 실제 발송은 그 HTML을
# `render_png`에 태우고, 거기서 **접힘·겹침·크기 게이트**를 통과해야 한다.
# 게이트에 걸린 카드는 오류를 내지 않고 **옛 v4 카드로 조용히 대체된다.**
# 그래서 검증 1,450건이 전부 통과하는데도 채널에는 옛 디자인이 나갔다.
#
# 실제로 걸린 것: v1.16에서 분석글을 상세하게 만들며 총평이 길어졌는데
# (`비교 4개 항목` → `8개 항목`), 그 문장을 담는 `.agv`가 '접혀도 되는 칸'
# 목록에서 빠져 있었다. 단일 경기 총평(`.vd p`)은 처음부터 예외였다.
# 문장 길이에 따라 걸리는 카드와 안 걸리는 카드가 갈려
# (실측: NPB 묶음0 통과 · 묶음1 탈락) "가끔 옛날 것이 섞인다"로 보였다.
#
# **같은 사고가 두 번째다** — 7.5절 주석의 NPB 분석도 같은 얼굴이었다.
# 그래서 이번에는 '만들어지는가'가 아니라 **'게이트를 지나는가'**를 친다.
_LONG_VERDICT = ("기록은 소프트뱅크 쪽이다. 8개 항목 중 6개에서 앞선다 — "
                 "순위 · 승률 · 최근 5경기 · 팀타율 · 팀평균자책 · 상대전적.")
_LONG_KEYS = ("승률 <b>0.559</b> : <b>0.610</b> · 팀타율 <b>0.271</b> : <b>0.279</b> · "
              "팀평균자책 <b>4.38</b> : <b>4.13</b>")
_long_rows = [{"no": i, "time": "18:30", "away": "소프트뱅크", "home": "니혼햄",
               "away_sub": "1위 · 78승 52패", "home_sub": "2위 · 74승 55패",
               "keys": _LONG_KEYS, "verdict": _LONG_VERDICT} for i in (1, 2, 3)]
_long_html = C5.shell(kind="analysis", league=League.NPB, date_label="9.8 화",
                      head=H.fallback("analysis"),
                      body=C5.body_analysis_multi(_long_rows),
                      foot_left="NPB 공식 기록")
try:
    import tempfile as _tf
    _png = pathlib.Path(_tf.mkdtemp()) / "gate.png"
    _r = R5.render_png(_long_html, _png)
except Exception as _e:                                           # noqa: BLE001
    _r = None
    check("★★★ 긴 총평이 실린 분석 카드가 렌더 게이트를 지난다", False,
          f"{_e.__class__.__name__}: {_e}")
else:
...
_orig_js = C5._MEASURE_JS
try:
    C5._MEASURE_JS = _orig_js.replace(",.agk,.agv", "")
    _broken = R5.render_png(_long_html, _png)
finally:
    C5._MEASURE_JS = _orig_js
check("  ↳ 변이: 총평을 예외에서 빼면 이 검사가 잡는다",
      not _broken, "깨뜨렸는데도 통과했다 — 이 검사가 헛돈다")

# 폴백이 일어나면 **조용하지 않아야 한다** (v1.17b).
# 위 변이에서 실제로 폴백이 났으므로 기록이 남아 있어야 한다.
_fb = R5.take_fallbacks()
check("  ↳ 폴백이 기록에 남는다 (틱이 이것을 운영 알림에 싣는다)",
      bool(_fb), "폴백이 났는데 아무 기록도 안 남았다")
check("  ↳ 기록을 거둬 가면 비워진다 (다음 틱에 또 알리지 않는다)",
      not R5.take_fallbacks())
...
# ══════════════════════════════════════════════════════════════
print("\n★ 한 카드에서 같은 말을 두 번 하지 않는다 (v1.54)")
# ══════════════════════════════════════════════════════════════
#
# 실측 2026-09-18: KBO 분석 카드가 1911~1982px로 **한계(2000)까지 18px**
# 남기고 돌았고, 일부는 넘어서 압축본으로 다시 그려졌다. 원인은 중복이다 —
# 최근 5경기 점그림과 맞대결 막대가 카드에 있는데, 바로 아래 예상글이 같은
# 값을 문장으로 또 적었다. 빼면 **같은 말이 사라지고 높이도 준다.**
import inspect as _insp54                                      # noqa: E402
_src54 = _insp54.getsource(R5.analysis_card)
check("★★★ 점그림을 그리면 예상글의 최근 폼 줄을 뺀다",
      "_draws_form" in _src54 and "not _draws_form" in _src54)
check("★★★ 맞대결 막대를 그리면 예상글의 맞대결 줄을 뺀다",
      "_will_draw_h2h" in _src54 and 'h2h_text = ""' in _src54)
check("★★ 점그림이 있으면 표의 `최근10` 줄을 뺀다 (같은 질문에 답한다)",
      '!= "최근10"' in _src54)
check("  ↳ 그래도 **세기에는 남긴다** (예상의 근거는 그대로다)",
      "row=False" in _src54 or "metrics.append" in _insp54.getsource(R5))
...
# 이르다"*라고 밝힌다(참고 2026-09-18). **밝히지 않는 수는 과장하는 수다.**
_st_small = {"name": "라일리", "era": "2.67", "whip": "1.00", "games": 19,
             "w": 7, "l": 1, "inn": "114 2/3", "kk": 145,
             "vs": {"games": 2, "era": "4.09"}}
_st_big = dict(_st_small, vs={"games": 12, "era": "3.10"})
_html_small = C5.body_starters("SSG", "NC", _st_small, _st_small)
_html_big = C5.body_starters("SSG", "NC", _st_big, _st_big)
check("★★★ 상대 표본이 3경기 이하면 그렇다고 적는다",
      "단정하기 이릅니다" in _html_small)
check("  ↳ 표본이 넉넉하면 군말을 안 붙인다",
      "단정하기 이릅니다" not in _html_big)
check("★★ 선발 비교에 경기수·WHIP·이닝·탈삼진이 들어간다 (이미 오던 값이다)",
      all(x in _html_small for x in ("19경기", "WHIP", "114 2/3이닝", "145K")),
      plain(_html_small)[:160])
check("★★ 한쪽만 있으면 대비를 만들지 않는다 (빈 칸은 그 팀이 없는 것처럼 읽힌다)",
      C5.body_starters("SSG", "NC", _st_small, None) == "")
...
# ══════════════════════════════════════════════════════════════
#
# 예상 한 줄은 **비교 항목이 셋 이상**이어야 뜬다(`headline.for_preview`).
# 팀 기록이 KBO 전용이라 나머지 리그는 순위·승률 둘뿐이었고,
# **실측 2026-09-18: 예정 경기 121장 중 예상이 붙은 카드 0장**이었다.
import render_v5 as _R43                                        # noqa: E402
class _Stk:
    """연속만 들고 있는 대역 — `_streak_num`·`_streak_text`가 보는 두 칸."""

    def __init__(self, kind, n):
        self.streak_kind, self.streak_len = kind, n


check("★★ 연승은 +, 연패는 − 로 한 축에 놓는다 (둘 다 3이면 무승부로 읽힌다)",
      _R43._streak_num(_Stk(StreakKind.WIN, 3)) == 3
      and _R43._streak_num(_Stk(StreakKind.LOSS, 3)) == -3)
check("  ↳ 한 경기는 '연속'이 아니다 (1연승이라고 쓰지 않는다)",
      _R43._streak_text(_Stk(StreakKind.WIN, 1)) == "1승"
      and _R43._streak_text(_Stk(StreakKind.WIN, 3)) == "3연승")
# **무승부 연속 (v1.71).** 축구에는 흔하고 `3무`는 실제 정보다. 버리면 그
# 팀 칸이 `—`가 되고, 양쪽이 다 `—`면 앵커의 흐름 줄이 통째로 사라진다
# (실측 2026-09-20: EPL 본머스 3연무가 그렇게 없어졌다).
check("★★ 무승부 연속도 적는다 (`3무` — 버리면 흐름 줄이 통째로 사라진다)",
      _R43._streak_text(_Stk(StreakKind.DRAW, 3)) == "3무",
      _R43._streak_text(_Stk(StreakKind.DRAW, 3)))
check("  ↳ 한 번 비긴 것은 '연속'이 아니다",
      _R43._streak_text(_Stk(StreakKind.DRAW, 1)) == "—")
check("★★ 무승부 연속은 **앞선 쪽이 아니다** (숫자축 0 — 승도 패도 아니다)",
      _R43._streak_num(_Stk(StreakKind.DRAW, 3)) == 0)
# 차례는 저장소 관례(승-패-무)다 — 한 상자 안에서 `0승 4무 1패`와
# `0승 1패 4무`가 나란히 나온 적이 있다(2026-09-18 실렌더).
check("★★ 무승부가 있으면 전적에 무를 적는다 · 차례는 승-패-무",
      _R43._wld_text((2, 1, 2)) == "2승 1패 2무"
      and _R43._wld_text((3, 1, 0)) == "3승 1패",
      _R43._wld_text((2, 1, 2)))
...
# ── ★★ **선발 맞대결의 이닝·탈삼진이 한 칸에 안 겹치는가** (v2.20 · 실제 사고) ──
#
# `소화` 한 줄에 `163 2/3이닝 · 137K` 를 같이 넣었다. 16자인데 `.cmp` 값 칸의
# 한 줄 한도가 **딱 16자**라(실측), 이름이 한 글자만 길어도 넘쳐 **2.1줄**이
# 되고 카드가 통째로 거절된다. 실측 2026-09-26: KBO 사전정보 4건이 그렇게
# 안 나갔다. 줄이지 않고 **나눴다** — 정보를 하나도 안 버린다.
_aw20 = {"name": "LG", "pitcher": "임찬규", "win": 11, "lose": 6,
         "era": "3.42", "whip": "1.28", "inn": "163 2/3", "kk": 137,
         "vs": {"games": 3, "era": "4.09"}}
_hm20 = {"name": "한화이글스", "pitcher": "류현진", "win": 9, "lose": 8,
         "era": "3.10", "whip": "1.19", "inn": "152 2/3", "kk": 161,
         "vs": {"games": 2, "era": "2.50"}}
_b20 = C5.body_starters("LG트윈스", "한화이글스", _aw20, _hm20)
check("★★ 이닝과 탈삼진이 **다른 줄**이다 (한 칸에 몰면 접힌다)",
      "소화" in _b20 and "탈삼진" in _b20
      and "이닝 · " not in _b20,
      "한 칸에 둘을 넣으면 16자 한도를 넘는다")
check("  ↳ 값은 하나도 안 버렸다 (이닝·탈삼진 둘 다 카드에 있다)",
      "163 2/3이닝" in _b20 and "137K" in _b20, _b20[:120])
