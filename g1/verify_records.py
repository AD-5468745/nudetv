...
check("★★★ **전부** 맞아야 순위를 만든다 (반쪽 표는 순위가 틀린 표다)",
      _FD.MIN_MAPPED_RATIO == 1.0, str(_FD.MIN_MAPPED_RATIO))

# ★ 대표님: *"발송누락도 절대 없어야해"*. 그래서 짝짓기가 **스스로 끝까지
#   풀려야** 한다 — 단독 경기만 세면 표가 아주 천천히 차고, 그동안 그 리그는
#   아무것도 안 나간다. 아는 팀으로 모르는 팀을 푼다.
_prop = _map([("2026-09-14T18:00:00", "MCI", "MUN"),
              ("2026-09-20T14:00:00", "MCI", "ARS"),
              ("2026-09-20T14:00:00", "LIV", "CHE")],
             [_FG("2026-09-14T18:00:00", "맨시티", "맨유"),
              _FG("2026-09-20T14:00:00", "맨시티", "아스널"),
              _FG("2026-09-20T14:00:00", "리버풀", "첼시")])
check("★★★ 아는 팀으로 **동시 경기**를 풀어낸다 (표가 빨리 찬다)",
      _prop.get("ARS") == "아스널", str(_prop))
