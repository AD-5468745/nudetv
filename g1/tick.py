...
def fetch_months(today: datetime) -> tuple[int, list[str]]:
    ...
    **월말에는 다음 달을 함께 긁는다 (v1.11f).**
    ...
    ── ★★ **이틀은 커버리지 판정에 모자랐다** (v2.21 · 실제 사고) ──────
    앞을 이틀만 보면 **월 하순에 남은 경기가 한두 건**이 된다. 그러면
    커버리지 감시의 완화 조건(`ahead >= FUTURE_OK_MIN` = "소스가 살아 있다는
    증거")이 꺼져 **없는 사고로 빨간불**이 켜진다 — 그리고 틱이 실패로 끝난다.
    실측 2026-09-26: K리그1 스냅샷이 09-27 까지뿐(앞으로 1경기)이라
    `[KL1] 오늘 편성이 사라짐` 이 빨간불로 올라왔다. 실제로는 10월 편성이
    멀쩡히 있는데 **우리가 안 받아온 것**이다.
    유럽에서 같은 병을 v2.00 으로 고쳤다(창 10일 → 50일). 여기도 같다.

    K리그는 주 1~2경기라 남은 경기 셋을 확보하려면 **2주**는 봐야 한다.
    그래서 다음 달을 **하순부터** 함께 긁는다. 리그 셋에 월 1회분 요청이
    늘 뿐이고(어댑터가 캐시한다), 대신 없는 사고로 울지 않는다.
    """
    y = today.year
    cands = [today.replace(day=1) - timedelta(days=1),   # 지난달
             today,
             today + timedelta(days=14)]                 # 하순이면 다음 달
...
def _poll_path():
    ...
    ⚠️ **옆에 두는 것만으로는 모자랐다** (v1.83에서 알아냄).
    `ROOT`는 `state/`라 경로 자체는 처음부터 맞았다. 그런데 시계가 깃에
    올리는 것은 **이름을 하나씩 적어 둔 몇 개뿐**이고(`tick.yml`의
    `git add`), 이 파일은 그 목록에 없었다. 그래서 한 실행 안에서는
    살아 있다가 **실행이 교대하면 통째로 사라졌고**, 다음 실행은 닫을
    투표의 글번호를 못 찾았다. 목록에 넣어 고쳤다 —
    **새 상태 파일을 만들 때는 그 목록도 같이 봐야 한다.**
    """
