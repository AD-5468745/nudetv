...
# ── 순위 경쟁 우선 (2026-09-06 대표님 결정) ────────────────────
#
# 전에는 1·2위가 1.0경기 넘게 벌어지면 곧장 연속·최근10으로 떨어졌고,
# **부진 기록이 호조 기록보다 통계적으로 더 극단적이라 꼴찌 팀의 부진이
# 순위표 카드의 얼굴이 되는 일이 반복됐다.** 순위표는 순위를 말하는 카드다.
#
# 여기 네 검사가 그 우선순위를 못박는다. 하나라도 어긋나면 옛 동작이다.
_race = [_st("KT", 1, 69, 44, 3, "0"), _st("SS", 2, 62, 52, 3, "6"),
         _st("LG", 3, 61, 53, 1, "6.5"), _st("HT", 4, 58, 56, 1, "9.5"),
         _st("OB", 5, 55, 59, 1, "12.5"), _st("NC", 6, 53, 61, 1, "14.5"),
         _st("HH", 9, 50, 65, 3, "20", last10=WLD(1, 9, 0))]
_h = H.for_standings(_race, KBO)
check("1·2위가 벌어져도 상위권 다툼이 있으면 그것이 헤드라인",
...
_runaway = [_st("KT", 1, 69, 44, 3, "0"), _st("SS", 2, 60, 60, 0, "9"),
            _st("LG", 3, 58, 62, 0, "11"),
            _st("HH", 9, 50, 65, 3, "20", last10=WLD(1, 9, 0))]
_h = H.for_standings(_runaway, KBO)
check("상위권이 전부 벌어졌으면 '독주'가 그날의 순위 이야기",
      _h and _h.rule == "S-LEAD" and _h.facts["gap"] == 9.0, str(_h))

# **순위 서사가 하나도 없을 때만** 기록으로 내려간다.
# 상위 절반(1~2위)이 1.0 초과 4.0 미만으로 벌어져 있어 S-RACE·S-LEAD 둘 다 안 걸린다.
_with10 = [_st("KT", 1, 69, 44, 3, "0"), _st("SS", 2, 66, 47, 3, "3"),
           _st("HH", 3, 50, 65, 3, "20", last10=WLD(1, 9, 0))]
_h = H.for_standings(_with10, KBO)
check("순위 서사가 없으면 그때 최근10 치우침을 잡는다",
      _h and _h.rule == "S-LAST10" and _h.facts["last10_win"] == 1, str(_h))

# **변이시험 — 깨뜨려서 잡히는지 본다.**
# 상위권 다툼을 없애면(전부 4경기 이상 벌림) S-RACE가 안 나와야 한다.
_nogap = [_st("KT", 1, 69, 44, 3, "0"), _st("SS", 2, 62, 52, 3, "6"),
          _st("LG", 3, 56, 58, 1, "12"), _st("HT", 4, 52, 62, 1, "16"),
          _st("OB", 5, 48, 66, 1, "20"), _st("NC", 6, 44, 70, 1, "24")]
check("  ↳ (변이) 상위권이 다 벌어지면 S-RACE는 안 나온다",
      (H.for_standings(_nogap, KBO) or H.Headline("", "")).rule != "S-RACE")
