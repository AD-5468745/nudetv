...
# 앞뒤는 보는 사람의 마음이 반대다:
# 앵커(경기 문패)가 서는 시각. 예고는 이것보다 먼저 나와야 한다.
ANCHOR_LEAD_SECONDS = 3 * 3600

#   · 경기 **전** — 미리 알고 싶다 → 예고는 **일찍**
#   · 경기 **후** — 빨리 알고 싶다 → 결과는 **바로** (경기별 속보가 즉시 나간다)
# 그래서 예고는 **앞으로 길게** · 정리판은 30분으로 **일부러 비대칭**이다.
#
# (처음 값은 120분이었다 — 옛 시작 알림이 쓰던 검증된 간격을 이어받은 것이다.
#  지금은 아래 v1.42 이유로 앵커에 매달아 **210분**이다.)
#
# ★ **예고는 그날 첫 앵커보다 먼저 나와야 한다** (v1.42 — 2026-09-17 실채널).
#
# 예고 120분 · 앵커 180분이라, 앵커가 **한 시간 먼저** 나갔다. 대표님이
# 채널에서 보시고 *"각 경기 앵커먼저 날라오고 이후에 전체예고가 날라오면
# 안되지 · 순서도 뒤죽박죽이야"*라고 하셨다. 맞는 지적이다 — 목차가 본문
# 뒤에 오는 셈이었다.
#
# 그래서 **앵커 리드에 매단다.** 숫자를 따로 적어 두면 한쪽만 바뀌는 날
# 순서가 다시 뒤집힌다. 30분은 예고와 첫 앵커 사이의 숨 쉴 틈이다.
PREVIEW_BEFORE_FIRST_SECONDS = ANCHOR_LEAD_SECONDS + 30 * 60

# 이 관계가 깨지면 채널이 다시 뒤죽박죽이 된다 — 숫자가 아니라 **관계**를 지킨다.
assert PREVIEW_BEFORE_FIRST_SECONDS > ANCHOR_LEAD_SECONDS, (
    "전체 예고가 앵커보다 늦게 나갑니다 — 채널 순서가 뒤집힙니다")
...
def _by_time(games: list) -> list:
    ...
    같은 시각끼리 묶는다. `_compose_index` 와 `game_link_lines` 가 **같은 것을
    쓴다** — 두 벌로 두면 한 화면은 묶이고 다른 화면은 안 묶인다.
    """
    out: list = []
    for g in sorted(games, key=lambda x: x.start_utc):
        t = f"{g.start_utc.astimezone(KST):%H:%M}"
...
def _compose_index(todays: list, day: str, lk: dict, nm,
    ...
    for lg in sorted(by, key=lambda x: order.get(x, 99)):
        ...
        # ── **같은 시각 경기는 시각을 한 번만 적는다** (v1.61) ──────
        #
        # 대표님(2026-09-19): *"경기마다 전부 시간이 적혀있어 눈아파."*
        # KBO 4경기가 전부 17:00이면 `17:00`이 네 번 찍힌다. 읽는 사람에게
        # 그 숫자는 **한 번만** 필요하다.
        #
        # 혼자 열리는 경기는 묶을 것이 없으므로 줄 안에 그대로 둔다 — 머리줄을
...
def build_queue(games: list[Game], now: datetime, channel: str,
    ...
    **멱등키에 리그를 반드시 넣는다 (v1.11c).**
    ...
    for _day, _bkey, _first in sorted(_buckets, key=lambda x: x[2]):
        # **심야 회피를 그대로 이어받는다.** 예고 시각이 한국시각 새벽이면
...
# ══════════════════════════════════════════════════════════════
#
# 대표님 지시(2026-09-10): 경쟁 채널 조사 · 보완 **1번**.
...
#
# 그쪽은 *"4경기 표본이라 단정하기 이르다"* 를 자동 생성 글에 적는다.
...
# ══════════════════════════════════════════════════════════════
#
# 대표님 지시(경쟁 채널 조사 · 보완 **3번**). v1.27이 야구 이닝으로 한 것을 축구에.
#
# ⛔ **카드에 있는 걸 텍스트에 또 쓰지 않는다.**
