...
이 프로젝트에서 실제로 그 계열의 사고가 세 번 있었다:
  ...
  · 위키 소스 레이트리밋을 0건으로 읽음 (에러가 HTTP 200으로 왔다)
  · 위키에서 리그 이름을 못 찾아 0건        (그 이름이 위키에 없었다)
...
# --- 정기 휴식일 (v1.11e) ---------------------------------------------------
# KBO·K리그1은 월요일에 경기를 하지 않는다. NPB도 월요일은 거의 없다.
...
class Finding:
    ...
    # 이걸로 시계를 실패 처리하면, 8월마다 비시즌 리그가 울어 진짜 사고가 그 소음에 묻힌다.
    soft: bool = False
...
def check_snapshot_age(fetch_log: dict, now: datetime,
    ...
    for name, rec in sorted(fetch_log.items()):
        ...
        # 발행에서 뺀 리그는 카드가 한 장도 안 나가는데,
        # 팬 위키 레이트리밋 때문에 2026-09-08~09 실측 **241틱 내리**
