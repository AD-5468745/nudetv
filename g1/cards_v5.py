...
KIND_META = {
    ...
    # v1.40 — **취소는 결과가 아니다.** 우천취소 경기가 `✅ 경기 결과`라는
    # 머리로 나갔다(2026-09-17 실측). 체크 표시는 "치러졌다"로 읽힌다.
    # 대표님 설계에도 취소는 따로 알리는 항목이다. 구름과 빗줄기 모양.
    "canceled": ("경기 취소", "M6.5 14a4 4 0 0 1 .5-8 5.5 5.5 0 0 1 10.4 1.6"
                              "A3.5 3.5 0 0 1 17 14zM8 17.5l-1 3M12 17.5l-1 3"
                              "M16 17.5l-1 3"),
    # 연기는 취소와 다르다 — **다시 열린다.** 그림도 갈라야 한다(검증이 잡았다:
    # 같은 도형을 쓰면 스크롤에서 취소와 연기가 구분되지 않는다).
    # 되감기는 화살표가 달린 시계 — '다시 잡힌다'를 도형으로 말한다.
    "postponed": ("경기 연기", "M20.5 12a8.5 8.5 0 1 1-2.5-6M20.5 3v5h-5"
                               "M12 7.5V12l3.5 2"),
    ...
    # v1.45 — 경기 기록실. 결과(체크)·경기전(명단)과 **또 다른 도형**을 쓴다:
    # 한 경기 댓글에 네 장이 연달아 붙으므로 그림이 겹치면 못 가린다. 장부.
    "boxscore": ("경기 기록실", "M5 3h11l3 3v15H5zM8 9h8M8 13h8M8 17h5"),
    ...
    # 이름이 `경기`이면 **아무것도 안 알려준다** — 모든 카드가 경기 얘기다.
    # 해시태그도 `#경기`가 되어 검색에 쓸모가 없다. 이 카드가 하는 일은
    # '이 경기의 문패'이므로 **맞대결**이라고 부른다(`#맞대결`로 모인다).
    "anchor":   ("맞대결", "M3 8h11l-3-3M21 16H10l3 3"),
}
...
def body_gameinfo(*, kst: str, local: str = "", venue: str = "",
    ...
    **점수를 다시 쓰지 않는다.** 머리말이 이미 "밀워키 8 : 12 신시내티"라고
    ...
    """
    when = esc(kst) + (f' <em>· 현지 {esc(local)}</em>' if local else "")
    # 라벨을 받는 이유 — **취소된 경기는 시작한 적이 없다.** `시작 18:30`은
    # 치러졌다는 뜻으로 읽힌다. 취소 카드는 `예정 18:30`이라고 쓴다.
    rows = [(time_label, when)] + [(k, esc(v)) for k, v in (extra or []) if v]
    if venue:
