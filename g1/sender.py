...
#
# 한 소스가 호출 한도에 걸려 캐시로 버티는 동안 알림 본문이
...
# ── ★ 날짜만은 뭉개지 않는다 (v1.21, 2026-09-09) ───────────────────
#
# **뭉개기가 너무 세면 어제 사고와 오늘 사고가 같은 말이 된다.**
#
# 실측(2026-09-09 관측): KBO 킥오프 누락이 이틀 연속 났는데 두 줄이
#
#   어제  큐에조차 들어오지 못한 킥오프 5건 — KBO:2026-09-08@18:30 (예약 18:00)
#   오늘  큐에조차 들어오지 못한 킥오프 4건 — KBO:2026-09-09@18:30 (예약 18:00)
#
# 뭉개면 **글자 그대로 같아진다**:
#
#   큐에조차 들어오지 못한 킥오프 <1>건 — KBO:<4>-<2>-<2>@<2>:<2> (예약 <2>:<2>)
#
# 그래서 17:45에 어제 것이 나간 뒤, 18:29에 생긴 **오늘의 진짜 누락이
# "방금 한 말"로 접혔다.** 감지는 정확히 잡았는데 대표님께 닿지 않았다.
#
# **매일 같은 시각에 반복되는 사고일수록 더 조용해지는 구조였다** — 정확히
# 반대여야 한다. 약점 117·125의 세 번째 얼굴이고 **방향이 반대다**:
# 그때는 뭉개기가 약해서 같은 말이 계속 나갔고, 이번엔 세서 다른 말이 묻혔다.
#
# 날짜는 '계속 변하는 수'가 아니다 — **날이 바뀌면 다른 사고다.**
# 그래서 `YYYY-MM-DD`만 원문 그대로 두고 나머지 수는 그대로 뭉갠다.
# (경과 시간 "2.0시간 → 2.2시간"은 여전히 뭉개져 도배가 안 난다.)
_FP_KEEP = re.compile(r"\d{4}-\d{2}-\d{2}")


def _fp_norm(text: str) -> str:
    """알림 지문용 정규화 — 수는 자릿수만 남긴다. **날짜는 남긴다.**"""
    def _bucket(m: "re.Match") -> str:
        whole = m.group(0).split(".")[0].replace(",", "")
        return f"<{len(whole) or 1}>"
    out: list[str] = []
    last = 0
    for keep in _FP_KEEP.finditer(text):
        out.append(_FP_NUM.sub(_bucket, text[last:keep.start()]))
        out.append(keep.group(0))          # 날짜는 그대로 둔다
        last = keep.end()
    out.append(_FP_NUM.sub(_bucket, text[last:]))
    return "".join(out)
...
class Ledger:
    ...
    def count_corrections_today(self, day_kst: str) -> int:
        ...
        **왜 따로 세나.** 정정은 `UNPLANNED_CONTENT`라 일반 하루 상한
        ...
        폭주 차단기는 10분 90건짜리 홍수만 잡지, 하루 종일 이어지는 정정 스무 건은
...
class Sender:
    def __init__(self, transport, ledger: Ledger, chat_id: str, *,
        ...
        # 그런데 면제만 해두면 정정에는 아무 뚜껑이 없다. 폭주 차단기(10분 90건)는
    ...
    def alert(self, title: str, lines: list[str], *, repeat_after: int | None = None) -> bool:
        ...
        **같은 내용은 되풀이하지 않는다 (v1.11d).**
        ...
        # 바뀐다** — 기록 수집은 30분마다, 흐름 보강은 또 다른 주기, 소스 재시도는
